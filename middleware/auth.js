...
const verifyApiKey = (req, res, next) => {
    ...
    db.get('SELECT * FROM api_keys WHERE api_key = ? AND is_active = 1', [apiKey], (err, row) => {
        if (err || !row) return res.status(401).json({ error: 'Invalid or inactive API key' });
        req.apiKey = row;
        db.run('UPDATE api_keys SET usage_count = usage_count + 1, last_used = CURRENT_TIMESTAMP WHERE id = ?', [row.id]);
        next();
    });
};
